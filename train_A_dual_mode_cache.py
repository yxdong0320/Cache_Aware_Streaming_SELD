...
from models.dual_mode_cache_resnet_conformer import DualModeResnetConformer, DualModeResnetConformer_Distill_on_Result
...
def main(args):
    ...
    if args['model']['type'] == 'Dualmode_seddoa_nopool':
        model = DualModeResnetConformer(in_channel=args['model']['in_channel'], 
    ...
    elif args['model']['type'] == 'Dualmode_seddoa_nopool_distill_on_result':
        model = DualModeResnetConformer_Distill_on_Result(in_channel=args['model']['in_channel'], 
                                                    in_dim=args['model']['in_dim'], 
                                                    out_dim=args['model']['out_dim'],
                                                    att_context_size = args['model']['att_context_size'],
                                                    num_conformer_layer = args['model']['num_conformer_layer'],
    ...
    while not stop_training:
        ...
        for data in train_dataloader:
            ...
            total_loss = stream_loss * args['train']['stream_weight'] + nonstream_loss * args['train']['nonstream_weight'] + distill_loss * args['train']['distill_weight']
            ...
            if step_count % args['result']['log_interval'] == 0:
                ...
                logger.info('epoch: {}, step: {}/{}, lr:{:.6f}, train_loss:{:.4f}, train_non_stream_loss:{:.4f}, train_stream_loss:{:.4f}'.format(epoch_count, step_count, total_steps, lr, np.mean(train_loss), np.mean(train_non_stream_loss), np.mean(train_stream_loss)))
