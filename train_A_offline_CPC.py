...
from models.resnet_conformer_CPC import ResnetConformer_sed_doa_nopool
...
def main(args):
    ...
    model = ResnetConformer_sed_doa_nopool(in_channel=args['model']['in_channel'], 
                                                ...
                                                cpc_future_steps=args['model']['cpc_future_steps'],
                                                negative_samples = args['model']['negative_samples'],
                                                )
    ...
    while not stop_training:
        ...
        for data in train_dataloader:
            ...
            optimizer.zero_grad()
            output, cpc_loss = model(input)
            loss = criterion(output, target)
            total_loss = loss + cpc_loss * args['train']['cpc_loss_weight']
            total_loss.backward()
            optimizer.step()
            scheduler.step()
            train_loss.append(loss.item())
            train_cpc_loss.append(cpc_loss.item())
            step_count += 1
            if step_count % args['result']['log_interval'] == 0:
                ...
                logger.info('epoch: {}, step: {}/{}, lr:{:.6f}, train_loss:{:.4f}, cpc_loss:{:.4f}'.format(epoch_count, step_count, total_steps, lr, loss.item(), cpc_loss.item()))
        ...
        for data in test_dataloader:
            ...
            with torch.no_grad():
                ...
                test_cpc_loss.append(cpc_loss.item())
