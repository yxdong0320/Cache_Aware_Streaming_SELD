...
from models.resnet_conformer_HPC import ResnetConformer_sed_doa_nopool
...
def main(args):
    ...
    while not stop_training:
        ...
        for data in train_dataloader:
            ...
            optimizer.zero_grad()
            output, (hpc_loss, cpc_loss, apc_loss) = model(input)
            loss = criterion(output, target)
            total_loss = loss + hpc_loss * args['train']['hpc_loss_weight']
            total_loss.backward()
            optimizer.step()
            scheduler.step()
            train_loss.append(loss.item())
            train_hpc_loss.append(hpc_loss.item())
            step_count += 1
            if step_count % args['result']['log_interval'] == 0:
                ...
                logger.info('epoch: {}, step: {}/{}, lr:{:.6f}, train_loss:{:.4f}, hpc_loss:{:.4f}'.format(epoch_count, step_count, total_steps, lr, loss.item(), hpc_loss.item()))
        ...
        for data in test_dataloader:
            ...
            with torch.no_grad():
                ...
                test_hpc_loss.append(hpc_loss.item())
