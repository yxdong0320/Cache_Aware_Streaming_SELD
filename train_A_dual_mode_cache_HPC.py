...
from models.dual_mode_cache_resnet_conformer_HPC import DualModeResnetConformerHPC, DualModeResnetConformerHPC_Distill_on_Result
...
def main(args):
    ...
    if args['model']['type'] == 'Dualmode_seddoa_nopool_APC':
        model = DualModeResnetConformerHPC(in_channel=args['model']['in_channel'], 
                                                    ...
                                                    hpc_future_steps=args['model']['hpc_future_steps'],
                                                    hpc_negative_samples=args['model']['hpc_negative_samples'])
    elif args['model']['type'] == 'Dualmode_seddoa_nopool_APC_distill_on_result':
        model = DualModeResnetConformerHPC_Distill_on_Result(in_channel=args['model']['in_channel'], 
                                                    in_dim=args['model']['in_dim'], 
                                                    out_dim=args['model']['out_dim'],
                                                    att_context_size = args['model']['att_context_size'],
                                                    num_conformer_layer = args['model']['num_conformer_layer'],
    ...
    while not stop_training:
        ...
        for data in train_dataloader:
            ...
            total_loss = stream_loss * args['train']['stream_weight'] + \
                            ...
                            total_hpc_loss * args['train']['hpc_weight']
            ...
            train_hpc_loss.append(total_hpc_loss.item())
            step_count += 1
            if step_count % args['result']['log_interval'] == 0:
                lr = optimizer.param_groups[0]['lr']
                logger.info('epoch: {}, step: {}/{}, lr:{:.6f}, train_loss:{:.4f}, train_non_stream_loss:{:.4f}, train_stream_loss:{:.4f}, train_hpc_loss:{:.4f}'.format(epoch_count, step_count, total_steps, lr, np.mean(train_loss), np.mean(train_non_stream_loss), np.mean(train_stream_loss), np.mean(train_hpc_loss)))
