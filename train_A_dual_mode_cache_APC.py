...
from models.dual_mode_cache_resnet_conformer_APC import DualModeResnetConformerAPC
...
def main(args):
    ...
    if args['model']['type'] == 'Dualmode_seddoa_nopool_APC':
        model = DualModeResnetConformerAPC(in_channel=args['model']['in_channel'], 
    ...
    while not stop_training:
        ...
        for data in train_dataloader:
            ...
            if args['train']['only_nonstream_apc']:
                total_apc_loss = apc_losses['nonstream_apc_loss']
            else:
                total_apc_loss = apc_losses['total_apc_loss']
            ...
            total_loss = stream_loss * args['train']['stream_weight'] + \
                            ...
                            total_apc_loss * args['train']['apc_weight']
            ...
            train_non_stream_apc_loss.append(apc_losses['nonstream_apc_loss'].item())
            train_stream_apc_loss.append(apc_losses['stream_apc_loss'].item())

            step_count += 1
            if step_count % args['result']['log_interval'] == 0:
                lr = optimizer.param_groups[0]['lr']
                # 输出 epoch， step， lr 和 以上六种loss的信息
                logger.info('epoch: {}, step: {}/{}, lr:{:.6f}, train_loss:{:.4f}, train_distill_loss:{:.4f}, train_stream_loss:{:.4f}, train_non_stream_loss:{:.4f}, train_non_stream_apc_loss:{:.4f}, train_stream_apc_loss:{:.4f}'.format(epoch_count, step_count, total_steps, lr, np.mean(train_total_loss), np.mean(train_distill_loss), np.mean(train_stream_loss), np.mean(train_non_stream_loss), np.mean(train_non_stream_apc_loss), np.mean(train_stream_apc_loss)))
        ...
        logger.info('epoch: {}, step: {}/{}, train_time:{:.2f}, test_time:{:.2f}, average_train_loss:{:.4f}, average_test_stream_loss:{:.4f}'.format(epoch_count, step_count, total_steps, train_time, test_time, np.mean(train_total_loss), np.mean(test_stream_loss)))
