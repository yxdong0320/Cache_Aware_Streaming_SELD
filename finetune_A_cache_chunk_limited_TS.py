...
from models.cache_resnet_conformer import ResnetConformer_sed_doa_nopool_TS, ResnetConformer_sed_doa_nopool_TS_after_conformer
...
def main(args):
    ...
    if args['model']['type'] == 'seddoa_nopool_ts':
        model = ResnetConformer_sed_doa_nopool_TS(in_channel=args['model']['in_channel'], 
    ...
    elif args['model']['type'] == 'seddoa_nopool_ts_after_conformer':
        model = ResnetConformer_sed_doa_nopool_TS_after_conformer(in_channel=args['model']['in_channel'], 
                                                    in_dim=args['model']['in_dim'], 
                                                    out_dim=args['model']['out_dim'],
                                                    att_context_size = args['model']['att_context_size'],
                                                    num_conformer_layer = args['model']['num_conformer_layer'],
    ...
    while not stop_training:
        ...
        for data in train_dataloader:
            ...
            if args['model']['type'] == 'seddoa_nopool_ts':
                ...
                loss = loss1 * args['train']['kl_gamma'][0] + kl_loss * args['train']['kl_gamma'][1]
            elif args['model']['type'] == 'seddoa_nopool_ts_after_conformer':
                pred, target_ts, representation_loss = model(input)
                loss1 = criterion(pred, target)
                loss = loss1 + args['train']['l1_loss_weight'] * representation_loss
        ...
        for data in test_dataloader:
            ...
            with torch.no_grad():
                if args['model']['type'] == 'seddoa_nopool_ts':
                    ...
                    output, target_ts, representation_loss = model(input)
                    loss1 = criterion(pred, target)
                    loss = loss1 + args['train']['l1_loss_weight'] * representation_loss
