...
from .APC import APCModule, CrossModalAPCModule
...
class DualModeResnetConformerCrossModalAPC(nn.Module):
    """
    集成了跨模态APC损失的双模态SELD模型
    """
    def __init__(
        self, 
        in_channel=7, 
        ...
        self.att_context_size = att_context_size
        self.cache_past_len = self.att_context_size[0]
        self.use_nonstream_apc = use_nonstream_apc
        ...
        # 跨模态APC模块 - 使用流式特征预测非流式的未来特征
        self.cross_modal_apc = CrossModalAPCModule(
            input_dim=self.encoder_dim,
            future_steps=apc_future_steps,
        )
        
        # 可选：非流式模型的自监督APC
        if self.use_nonstream_apc:
            self.nonstream_apc = APCModule(
                input_dim=self.encoder_dim,
                future_steps=apc_future_steps,
                loss_f = loss_f
    ...
    def forward(self, x, mode='dual', resnet_cache=None, conformer_cache=None):
        ...
        Returns:
            根据mode返回不同的结果:
            - 'dual': 返回{'stream_pred', 'nonstream_pred', 'distill_loss', 'apc_losses'}
            - 'streaming' + 缓存: 返回(pred, (next_resnet_cache, next_conformer_cache))
            - 'streaming' 无缓存 或 'non-streaming': 返回pred
        """
        # 双模式 - 同时进行流式和非流式前向传播（主要用于训练）
        if mode == 'dual':
            ...
            # distill_loss = F.mse_loss(stream_conformer_out, nonstream_conformer_out.detach())
            
            # 计算跨模态APC损失 - 使用流式特征预测非流式的未来特征
            cross_modal_apc_loss = self.cross_modal_apc(stream_conformer_out, nonstream_conformer_out)
            
            # 汇总APC损失
            apc_losses = {
                'cross_modal_apc_loss': cross_modal_apc_loss,
            }
            
            # 可选：添加非流式自监督APC损失
            if self.use_nonstream_apc:
                nonstream_apc_loss = self.nonstream_apc(nonstream_conformer_out)
                apc_losses['nonstream_apc_loss'] = nonstream_apc_loss
                apc_losses['total_apc_loss'] = cross_modal_apc_loss + nonstream_apc_loss
            else:
                apc_losses['total_apc_loss'] = cross_modal_apc_loss
...
class DualModeResnetConformerCrossModalAPC_Distill_on_every_hidden(nn.Module):
    ...
    def forward(self, x, mode='dual', resnet_cache=None, conformer_cache=None):
        ...
        if mode == 'dual':
            ...
            for layer in self.conformer_layers:
                nonstream_conformer_out = layer(nonstream_conformer_out, mode='non-streaming')
                nonstream_conformer_outputs.append(nonstream_conformer_out.detach())
