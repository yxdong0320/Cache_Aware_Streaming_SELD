...
class CausalAttention(nn.Module):
    ...
    def forward(self, x, mask=None, cache=None, return_attn_weights=False):
        # 区分训练和推理模式
...
class ConformerBlock(nn.Module):
    ...
    def forward(self, x, mask=None, cache=None, return_attention=False):
        if cache is None:
            x = self.ff1(x) + x
            
            if return_attention:
                attn_out, attn_weights = self.attn(x, mask=mask, return_attn_weights=True)
                x = attn_out + x
            else:
            ...
            if return_attention:
                attn_out, attn_weights, next_attn_cache = self.attn(
                    x, mask=mask, cache=attn_cache, return_attn_weights=True
                )
                x = attn_out + x
            else:
            ...
            if return_attention:
                return (x, attn_weights), (next_attn_cache, next_conv_cache)
            else:
