...
class CausalAttention(nn.Module):
    ...
    def forward(self, x, context = None, mask = None, context_mask = None):
        ...
        # causal relative positional embedding
        seq = torch.arange(n, device = device)
        dist = rearrange(seq, 'i -> i ()') - rearrange(seq, 'j -> () j')
        # pdb.set_trace()
        # causal
        # dist = dist.clamp(-max_pos_emb, 0) + max_pos_emb
        dist = dist.clamp(-max_pos_emb, max_pos_emb) + max_pos_emb
        # 3-look-ahead 
        # dist = dist.clamp(-max_pos_emb, 3) + max_pos_emb
        ...
        # 3-look-ahead
        # causal_mask = torch.triu(torch.ones(n, n, device=device), diagonal=3).bool()
        ...
        chunk_idx = torch.arange(0, n, dtype=torch.int, device=att_mask.device)
...
class ConformerBlock(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.attn = CausalAttention(dim = dim, dim_head = dim_head, heads = heads, dropout = attn_dropout)
        self.conv = ConformerConvModule(dim = dim, causal = True, expansion_factor = conv_expansion_factor, kernel_size = conv_kernel_size, dropout = conv_dropout)
