...
class Attention(nn.Module):
    ...
    def forward(self, x, context = None, mask = None, context_mask = None, return_attn_weights = False):
        n, device, h, max_pos_emb, has_context = x.shape[-2], x.device, self.heads, self.max_pos_emb, exists(context)
...
class ConformerBlock(nn.Module):
    ...
    def forward(self, x, mask = None, return_attention = False):
        ...
        if return_attention:
            # 这里需要修改PreNorm来支持传递参数
            if isinstance(self.attn, PreNorm):
                # 直接访问底层函数
                x_norm = self.attn.norm(x)
                attn_out, attn_weights = self.attn.fn(x_norm, mask=mask, return_attn_weights=True)
                x = attn_out + x
            else:
