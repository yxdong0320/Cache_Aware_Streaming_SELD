...
from .HPC import HPCModule

class DualModeResnetConformerHPC(nn.Module):
    """
    集成了HPC损失的双模态SELD模型
    """
    def __init__(
        ...
        self.stream_hpc = HPCModule(
            input_dim=self.encoder_dim,
            future_steps=hpc_future_steps,
            negative_samples=hpc_negative_samples
        )
        
        self.nonstream_hpc = HPCModule(
            input_dim=self.encoder_dim,
            future_steps=hpc_future_steps,
            negative_samples=hpc_negative_samples
        )
    ...
    def forward(self, x, mode='dual', resnet_cache=None, conformer_cache=None):
        ...
        if mode == 'dual':
            ...
            # 计算HPC损失
            stream_hpc_loss, stream_apc_loss, stream_cpc_loss = self.stream_hpc(stream_conformer_out)
            nonstream_hpc_loss, nonstream_apc_loss, nonstream_cpc_loss = self.nonstream_hpc(nonstream_conformer_out)
            
            # 汇总HPC损失
            hpc_losses = {
                'stream_hpc_loss': stream_hpc_loss,
                'stream_apc_loss': stream_apc_loss,
                'stream_cpc_loss': stream_cpc_loss,
                'nonstream_hpc_loss': nonstream_hpc_loss,
                'nonstream_apc_loss': nonstream_apc_loss,
                'nonstream_cpc_loss': nonstream_cpc_loss,
                'total_hpc_loss': stream_hpc_loss + nonstream_hpc_loss
            }
            # 处理非流式输出
        ...
        # 仅流式模式
            ...
            # 无缓存的流式前向传播（用于评估或单次处理）
                ...
                # 计算HPC损失（如果在评估时需要）
                if self.training:
                    hpc_loss, apc_loss, cpc_loss = self.stream_hpc(conformer_out)
                ...
                if self.training:
                    return pred, {'hpc_loss': hpc_loss, 'apc_loss': apc_loss, 'cpc_loss': cpc_loss}
                return pred
        ...
        # 仅非流式模式
            ...
            # 计算HPC损失（如果在评估时需要）
            if self.training:
                hpc_loss, apc_loss, cpc_loss = self.nonstream_hpc(conformer_out)
...
class DualModeResnetConformerHPC_Distill_on_Result(nn.Module):
    """
    集成了HPC损失的双模态SELD模型
    """
    def __init__(
