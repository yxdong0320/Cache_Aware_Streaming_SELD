...
from models.cache_resnet_conformer_change_cache_len import ResnetConformer_sed_doa_nopool
...
def main(args):
    ...
    # 添加对推理缓存长度的配置支持
    inference_cache_len = args['model'].get('inference_cache_len', None)
    if inference_cache_len is not None:
        logger.info(f"Using custom inference cache length: {inference_cache_len}")
    ...
    model = ResnetConformer_sed_doa_nopool(in_channel=args['model']['in_channel'], 
                                           ...
                                           inference_cache_len=inference_cache_len)  # 添加推理缓存长度参数
    ...
    # 打印使用的缓存长度信息
    actual_cache_len = model.cache_past_len
    logger.info(f"Model using cache length: {actual_cache_len} (original: {args['model']['att_context_size'][0]})")
    print(f"Model using cache length: {actual_cache_len} (original: {args['model']['att_context_size'][0]})")
    ...
    logger.info('Cache Length: {}, ER/F/LE/LR/SELD: {}'.format(
        actual_cache_len,
        '{:0.4f}/{:0.4f}/{:0.4f}/{:0.4f}/{:0.4f}'.format(val_ER, val_F, val_LE, val_LR, val_seld_scr)))
    print('Cache Length: {}, ER/F/LE/LR/SELD: {}'.format(
        actual_cache_len,
        '{:0.4f}/{:0.4f}/{:0.4f}/{:0.4f}/{:0.4f}'.format(val_ER, val_F, val_LE, val_LR, val_seld_scr)))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cache_len', type=int, default=None, help='Custom inference cache length')
    input_args = parser.parse_args()
    # 不同任务使用不同配置文件
    with open(os.path.join('config', '{}.yaml'.format(input_args.config_name)), 'r') as f:
        args = yaml.safe_load(f)
    
    # 如果命令行参数提供了cache_len，则覆盖配置文件中的设置
    if input_args.cache_len is not None:
        if 'inference_cache_len' not in args['model']:
            args['model']['inference_cache_len'] = input_args.cache_len
