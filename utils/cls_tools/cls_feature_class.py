...
class FeatureClass:
    ...
    def _load_audio(self, audio_path):
        # if 'fold4_room23_mix002' in audio_path:
        #     print("Found target wav_name:", audio_path)
        #     pdb.set_trace()
...
#            pool.join()
    ...
    def segment_labels(self, _pred_dict, _max_frames):
        ...
        # nb_blocks = int(np.ceil(_max_frames))
        output_dict = {x: {} for x in range(nb_blocks)}
        for frame_cnt in range(0, _max_frames, self._nb_label_frames_1s):
        # for frame_cnt in range(0, _max_frames, 1):
    ...
    def get_label_dir(self):
        if self._is_eval:
            ...
            if self._spatial_map:
                flag = '{}_spmap'.format(self._dataset_combination)
            elif self._multi_accdoa:
                flag = '{}_adpit'.format(self._dataset_combination)
            else:
                flag = self._dataset_combination
            return os.path.join(
                self._feat_label_dir,
               '{}_label'.format(flag)
        )
